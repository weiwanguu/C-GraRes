...
class RawCNNLight1D(nn.Module):
    def __init__(self, num_classes=25):
        super().__init__()
        self.features = nn.Sequential(
            nn.Conv1d(36, 16, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm1d(16),
...
def main():
    ...
    model_name = "CNN-light"
    model = RawCNNLight1D(num_classes=NUM_OBJECTS).to(device)
